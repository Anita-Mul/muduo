...
Inspector::Inspector(EventLoop* loop,
                     ...
                     const string& name)
    : server_(loop, httpAddr, "Inspector:"+name),       // HTTP 服务器
      processInspector_(new ProcessInspector)
{
    ...
    processInspector_->registerCommands(this);          // 注册一些命令，proc 的那些
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                string command = result[1];                                     // opened_files
                /*
                        CommandList
                ...
                */
                ...
                CommandList::const_iterator it = commList.find(command);        // 查找command对应的命令  opened_files - callback
